    ...
    <div className="grid grid-cols-12 justify-center items-center bg-blue-50 bg-opacity-30">
      <div className="grid grid-cols-10 justify-center items-center col-span-12 px-20 py-20 rounded-sm bg-blue-100 bg-opacity-50">
        ...
        <span className="flex col-span-2 col-start-12">
          ...
          />
          <i class="ri-search-line -ml-2 bg-black px-4 py-2 rounded-tr-md rounded-br-md text-white shadow-[0_2px_10px_10px_rgba(0,0,0,0.1)] hover:bg-gray-400 hover:text-black"></i>
        </span>
      </div>
      <div className="grid grid-cols-12 justify-center items-center col-span-12 col-start-2 px-8 py-8">
        {categories.map((key, element) => (
          <div
            key={key}
            className="flex col-span-2 justify-center mx-2 px-2 py-2 rounded-sm hover:bg-gray-700 hover:text-white items-center bg-gray-200 text-gray-700 font-extralight"
          >
            {categories[element]}
          </div>
        ))}
      ...
      <div className="grid grid-cols-12 col-span-12 col-start-1 justify-center items-center px-2 py-2 gap-4 my-8">
        <div className="grid grid-cols-12 col-span-12 col-start-1 justify-center gap-4 items-center my-8 mx-8">
          {list.map((element) => (
            <Link
              to={element.to}
              className="flex flex-col col-span-4 px-4 py-4 bg-white border-2 border-gray-100 shadow-2xl shadow-gray-400 rounded-md transition ease-in delay-0 hover:-translate-y-2 duration:1000"
            >
              {element.courseCard.map((elements) => (
                <div className="flex flex-col">
                  ...
                  <i
                    ...
                    onClick={() => setFavorite(elements.fav)}
                  ></i>
                  ...
                  <span className="flex text-2xl text-black leading-6 my-4">
                    {elements.courseName}
                  </span>
                  <div className="flex justify-between items-center ">
                    {elements.info.map((ele) => (
                      <span className={ele.class}>
                        {ele.instructor}
                        {ele.actPrice}
                        {ele.disPrice}
                      </span>
